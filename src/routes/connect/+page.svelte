<script>
    import StravaAuth from "$lib/components/core/StravaAuth.svelte";
...
{#if $page.url.searchParams.get("error")}
    <div class="error">
        <div class="modal">
            ...
            <div class="close" on:click={() => goto("/connect", { replaceState: true })}>
                <X />
            </div>
            {#if $page.url.searchParams.get("error") == "access_denied"}
                <h1>Access Denied</h1>
                <p>You have denied access to your Strava account. Please try again.</p>
            {:else if $page.url.searchParams.get("error") == "invalid_scope"}
                <h1>Invalid Permissions</h1>
                <p>Please select all options when signing in with Strava.</p>
            {:else}
...
>
    ...
    <section>
        ...
        <button class="large light">I don't have Strava</button>
    </section>
</main>
